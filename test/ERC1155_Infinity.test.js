...
const { shouldBehaveLikeERC1155 } = require('./ERC1155_Infinity.behavior');
const ERC1155Mock = artifacts.require('contracts/standards/ERC1155.sol:ERC1155');
...
contract('ERC1155', function (accounts) {
  ...
  beforeEach(async function () {
    this.token = await ERC1155Mock.new();
  });
  ...
  describe('internal functions', function () {
    ...
    describe.skip('_mintBatch', function () {
      it('reverts with a zero destination address', async function () {
        await expectRevert(
    ...
    describe('_burnBatch', function () {
      ...
      it.skip('reverts if length of inputs do not match (MOVED TO MAIN CONTRACT)', async function () {
        await expectRevert(
      ...
      context('with minted-then-burnt tokens', function () {
        beforeEach(async function () {
          for (let index = 0; index < tokenBatchIds.length; index++) {
            await this.token.$_mint(tokenBatchHolder, tokenBatchIds[index], mintAmounts[index], data);
          }
  ...
  describe.skip('ERC1155MetadataURI', function () {

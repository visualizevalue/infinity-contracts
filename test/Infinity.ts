...
export const deployContract = deployments.createFixture(async ({ deployments, ethers }) => {
  ...
  const InfinityInterface = await deployments.get("Infinity");
  const contract = await ethers.getContractAt("Infinity", InfinityInterface.address);
...
describe("Infinity", () => {
  ...
  describe(`Generating`, () => {
    it(`Shouldn't allow minting for free`, async () => {
      await expect(contract.connect(vv).generate(addr1.address, "")).to.be.reverted;
    });

    it(`Should allow minting without a message`, async () => {
      await expect(contract.generate(addr1.address, "", { value: PRICE })).to.emit(
        contract,
        "TransferSingle",
      );
    ...
    it(`Should allow minting with a message`, async () => {
      await expect(
        contract.generate(addr1.address, "The beginning of infinity", {
          value: PRICE,
        }),
      )
    ...
    it(`Should allow minting many of the same token`, async () => {
      ...
      while (amount < 512) {
        await expect(
          contract.generate(addr1.address, "", {
    ...
    it(`Should allow minting many different tokens`, async () => {
      const tx = await contract
        .connect(addr1)
        .generateMany([addr1.address, addr1.address], [9, 19], {
    ...
    it(`Should allow transferring assets`, async () => {
      const tx = await contract
        .connect(addr2)
        .generate(addr2.address, "", { value: PRICE * BigInt(10) });
      ...
      await expect(
        contract.connect(addr2).safeTransferFrom(addr2.address, addr3.address, ID, 1, toBeArray(0)),
      )
        .to.emit(contract, "TransferSingle")
    ...
    it(`Shouldn't allow minting batches with an invalid deposit`, async () => {
      await expect(
        contract.generateMany([addr1.address, addr2.address], [10, 1], {
    ...
    it(`Shouldn't let users overpay when generating many`, async () => {
      await expect(
        contract.generateMany([addr1.address, addr2.address], [10, 1, 1], {
    ...
    it(`Shouldn't let users define more recipients than amounts`, async () => {
      await expect(
        contract.generateMany([addr1.address, addr2.address, addr3.address], [10, 1], {
    ...
    it(`Should allow minting and transferring batches`, async () => {
      ...
      await expect(
        contract
          .connect(addr4)
          .safeBatchTransferFrom(addr4.address, addr5.address, [ID1, ID2], [5, 4], toBeArray(0)),
      )
    ...
    it(`Should mark tokens as minted when VV mints them`, async () => {
      await expect(
        contract.connect(vv).generateExisting(ZeroAddress, vv.address, 0, "", { value: PRICE }),
      )
        .to.emit(contract, "TransferSingle")
        .withArgs(vv.address, ZeroAddress, vv.address, 0, 1);

      await expect(
        contract.connect(vv).generateExisting(ZeroAddress, owner.address, 2, "", { value: PRICE }),
      )
        .to.emit(contract, "TransferSingle")
      ...
      await expect(
        contract.connect(addr3).generateExisting(owner.address, addr3.address, 1, "", {
          value: PRICE,
        }),
      )
        .to.emit(contract, "TransferSingle")
      ...
      await expect(
        contract.connect(addr3).generateExisting(owner.address, addr3.address, 2, "", {
          value: PRICE,
        }),
      )
        .to.emit(contract, "TransferSingle")
  ...
  describe(`ReGenerating`, () => {
    it(`Should not allow regenerating tokens we don't own`, async () => {
      ...
      await contract.connect(vv).generateExisting(ZeroAddress, addr5.address, 7, "", {
        value: PRICE * BigInt(2),
      });

      await expect(contract.connect(addr4).regenerate(7, 1)).to.be.revertedWith(
        `ERC1155: burn amount exceeds balance`,
      );
      await expect(contract.connect(addr5).regenerate(7, 5)).to.be.revertedWith(
        `ERC1155: burn amount exceeds balance`,
      );
    ...
    it(`Should allow regenerating tokens`, async () => {
      await contract.connect(vv).generateExisting(ZeroAddress, vv.address, 0, "", { value: PRICE });
      await contract.connect(vv).generateExisting(ZeroAddress, addr5.address, 8, "", {
        value: PRICE * BigInt(2),
      });
    ...
    it(`Should allow regenerating multiple tokens at once`, async () => {
      await contract
        .connect(vv)
        .generateExisting(ZeroAddress, vv.address, 19, "", { value: PRICE });
      ...
      await expect(contract.connect(addr5).regenerateMany([9, 10], [2, 3])).to.be.revertedWith(
        `ERC1155: burn amount exceeds balance`,
      );

      await expect(await contract.connect(addr5).regenerateMany([9, 10], [2, 2]))
        .to.emit(contract, "TransferSingle")
  ...
  describe(`DeGenerating`, () => {
    it(`Should allow degenerating`, async () => {
      const TOKEN = 0;
      await contract.connect(vv).generateExisting(ZeroAddress, addr3.address, TOKEN, "", {
    ...
    it(`Should allow withdrawing funds with specified token amount`, async () => {
      const TOKEN = 0;
      await contract.connect(vv).generateExisting(ZeroAddress, addr2.address, TOKEN, "", {
        value: PRICE * BigInt(10),
      });
      await expect(contract.connect(addr2).degenerate(TOKEN, 50)).to.be.revertedWith(
        `ERC1155: burn amount exceeds balance`,
      );

      expect(await contract.connect(addr2).degenerate(TOKEN, 5)).to.changeEtherBalance(
        addr2,
        PRICE * BigInt(5),
      );
    });
    ...
    it(`Should allow withdrawing funds for many tokens`, async () => {
      ...
      expect(
        await contract.connect(addr5).degenerateMany([TOKEN, TOKEN + 1], [5, 5]),
